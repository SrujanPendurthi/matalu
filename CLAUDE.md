...
- **The cleanup model is QLoRA fine-tuned, and the adapter *is* committed** (`sidecar/adapters/cleanup/`, 21 MB). It is load-bearing: without it the app silently runs the base model, which captures 12.6% of the achievable cleanup instead of 76.1%, and the only symptom is one startup log line. Confirm which is loaded from `system prompt: short (tuned)` vs `full (base model)`. Intermediate training checkpoints *are* gitignored — they duplicate `adapters.safetensors`.
  Retraining (~10 min) is only needed after changing the data or prompt:
  ```bash
...
- **Do NOT `mlx_lm.fuse` the cleanup adapter. Four fusion variants were measured; all four lose.** Note this table is a **100-pair subset**, not the 250-pair split above — that is why the shipping row reads 2.84% here and 3.27% there. Compare rows within a table, never across:
...
- **Packaging bundles *both* sidecars — `sidecar/build_sidecar.sh` builds and stages them.** `externalBin` lists `binaries/matalu-sidecar` (ASR, `matalu-sidecar.spec`) and `binaries/matalu-cleaner` (cleanup, `matalu-cleaner.spec`). The cleaner bundle is ~364 MB and **carries the QLoRA adapter inside it** via `datas`; `clean_sidecar.py::_resource_dir()` resolves `sys._MEIPASS` when frozen so `resolve_adapter()` finds it. Verified: the built binary logs `system prompt: short (tuned)` and cleans correctly.
  - **The adapter is why this needs a guard, not just a build step.** Cleanup is best-effort — the app pastes raw text whenever the cleaner is unavailable — so a bundle missing the adapter would look like it worked while silently running the base model (12.6% of the achievable cleanup instead of 76.1%). Two defenses: `matalu-cleaner.spec` refuses to build if `sidecar/adapters/cleanup/` is absent, and `build_sidecar.sh` runs the built binary and fails unless it reports the tuned prompt. Both were tested against a deliberately adapter-less build.
  - The model itself is **not** bundled (same as ASR): first run downloads and caches from the Hub, so a packaged app needs network once.
  - `Cleaner::new` prefers the bundled binary via the shared `pipeline::bundled_binary(name)`, falling back to `python3 clean_sidecar.py`; an explicit `MATALU_CLEAN_SIDECAR` beats both.
  How the ASR half works: `tauri.conf.json` `bundle.externalBin` ships `binaries/matalu-sidecar-<triple>`; `sidecar/build_sidecar.sh` + `matalu-sidecar.spec` build it with **PyInstaller** (bundling mlx/parakeet-mlx is the fragile part — verify it loads on a clean machine). A committed **placeholder stub** at that path keeps `cargo build`/`cargo tauri dev` working and errors loudly if a packaged build ships without the real binary; the build script overwrites it (don't commit the built binary). At runtime `pipeline.rs` prefers a `matalu-sidecar` next to the app executable and falls back to the dev `python3` path — same prefer-bundled-else-dev shape as the model resolution.
- **The sidecar prefers a local/bundled model over the Hub** (`resolve_model()` in `matalu_sidecar.py`). Precedence: `MATALU_MLX_MODEL` (explicit override — local path or Hub repo ID) → a bundled `sidecar/models/parakeet-tdt-0.6b-v2/` dir if present (fully offline; the norm) → the Hub repo ID `mlx-community/parakeet-tdt-0.6b-v2` (downloads + caches on first run). Drop the model folder into `sidecar/models/` to run offline with no env var. The sidecar additionally reads **`MATALU_MLX_CONTEXT`** (default `128,128`) — the streaming attention context `(left,right)`, the main speed/accuracy knob. Measured here: `64,64` runs fast but **drops the utterance start** (too little warmup context); `128,128` recovers it and stays real-time (~1.1x RTF); `256,256` is no more accurate and decodes below real time (~0.7x, lags). Too small (e.g. `32,16`) collapses. User-facing prefs live in `settings.rs`; ASR/sidecar params stay env-driven.
